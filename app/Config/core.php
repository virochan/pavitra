 ...
 */
Configure::write('Security.salt', 'DYhG93b0qyJfIx90fs2guVoUubWwvniR2G0FgaC9mi');

/**
 * A random numeric string (digits only) used to encrypt/decrypt strings.
 */
Configure::write('Security.cipherSeed', '7685930969057453542496749683645');
...
$date_date = date('d');
if ($date_month >= 07) {
    if (($date_date >= 15) && ($date_month == 07)) {

        $global_ac_year = ($date_year - 1 ) . "-" . (substr($date_year, 2, 4));
    } else {

        $global_ac_year = ($date_year) . "-" . (substr($date_year, 2, 4) + 1 );
    }
} else {
    $global_ac_year = ($date_year - 1 ) . "-" . (substr($date_year, 2, 4));
}

 $global_ac_year = '2016-17';
Configure::write('global_ac_year', $global_ac_year);
