...
<?php echo $this->Form->create('pv_rost_unverf', array('url' => array('controller' => 'Pavitras', 'action' => 'unverify_roster'))); ?>
...
<div style="height:595px;width:100%;padding:5px;clear:both;">
    <div class="form_content" align="center">
        <div class="map_head" style="min-height:115px;height:auto">
            <h3> Un-verify Roster entry for Sanstha</h3>
            <table class="table b_table" width="100%" id="eoroster">
                ...
                <tr>
                    ...
                    <td class="col-xs-2" colspan="2"><?php echo $this->Form->input('rst_last_upd_dt', array('label' => false, 'autocomplete' => 'off', 'id' => 'datepicker', 'type' => 'text', 'placeholder' => 'DD/MM/YYYY', 'maxlength' => '30')); ?>   </td>
...
<!--                        <td width="8%">
                        ...
                        <td width="8%">
                            <span class="clus_btn">
                                Un-verify &nbsp;<input type="checkbox"  id="selectuvfy" value="U" name="select"/> 
                            </span>
                        </td>
                    </tr>
                </table>
        ...
        <div style="margin-top:5px;clear:both;" id="main_table">
            <table class="table table_grid" border="0" style="width:75%;background:#fff;">
                <tr  class="">
                    ...
                    <td class="th_grid col-xs-3" colspan="3"><div align="center" class="clorclas">Total Working Staff Approved</div></td>
...
//        });
        ...
        $('#pv_rost_unverfSancSum').attr('readonly', 'readonly');
        $('#pv_rost_unverfWrkSum').attr('readonly', 'readonly');
        $('#pv_rost_unverfVacntSum').attr('readonly', 'readonly');
        $('#resonid').hide();
        var numpattern = /^[0-9]*$/;
        ...
        function json_data() {
            ...
            jQuery.post(window.webroot + 'Pavitras/get_roster_unverf_data', {session_user_id: session_user_id, staff_group: staff_group,
...
//                                    alert(val);
                ...
                if (roster_file_name) {
                    $('#mycontainer1').show();
                    $('#view').show();
                    $("#pv_rost_unverfUplodimg").val(roster_file_name);
                    $('#close').show();
...
//                    $("#popup").attr("src", window.webroot + 'STADMIN_UPLOADS/' + roster_file_name + '?' + d.getTime());
                ...
                $('#pv_rost_unverfSancSum').val(p_sum);
                $('.work').each(function () {
                    if ($(this).val() != '') {
                        w_sum += Number($(this).val());
                    }
                });
                $('#pv_rost_unverfWrkSum').val(w_sum);
                $('.vcnt').each(function () {
                    if ($(this).val() != '') {
                        v_sum += Number($(this).val());
                    }
                });
                $('#pv_rost_unverfVacntSum').val(v_sum);
                $('#pv_rost_unverfRosterRemarks').val('');
                $('input:[type=checkbox][name=select]').attr('checked', false);
                $("input:[type=checkbox][name=select]").removeAttr("disabled");
                $('#resonid').hide();
                $('#pv_rost_unverfJsonFlag').val(asst_flag);
                /*Changed on 21/11/17*/
//                if (asst_flag == 'V') {
//                    $('input:[type=checkbox][id=selectuvfy]').attr('checked', true);
...
//                    //	$('input:[id=selectuvfy]').prop('disabled', 'disabled');
//                    $("#selectuvfy").attr("disabled", true);
//                    $('#resonid').show();
//                    $('#pv_rost_unverfRosterRemarks').val(roster_remarks);
//                    $('#pv_rost_unverfRosterRemarks').attr('readonly', 'readonly');
//                }
        ...
        $("#roster_submit").click(function () {
            ...
            if (!$('input:radio[name=staff_type]:checked').val() == '1') {
                ...
                var rejection_flag = $('#pv_rost_unverfAsstFlag').val();
                var remarks = $('#pv_rost_unverfRosterRemarks').val();
                if (rejection_flag == "R") {
                    if (remarks == "") {
                        flag = 0;
                        alert("Err...Enter Reason for Rejection.");
                        $("#pv_rost_unverfRosterRemarks").focus();
                        $('#pv_rost_unverfRosterRemarks').css('border-color', 'red');
                    } else {
                        if (window.confirm("Are you sure want to Reject Record ?")) {
                            $("#pv_rost_unverfRosterPvtTchVerfForm").submit();
                        }
                    }
                } else {
                    if (window.confirm("Are you sure want to Un-verify Record ?")) {
                        $("#pv_rost_unverfRosterPvtTchVerfForm").submit();
//                        location.reload();
                    }
                }
...
//        $("#pv_rost_unverfRosterPvtTchVerfForm").ajaxForm({url: 'verify_roster', type: 'post',
//            success: function(data) {
//                alert("Data Saved Successfully.");
////                location.reload();
//                $('#selectuvfy').attr('checked', false);
...
//            }
//        });
        $("#selectuvfy").change(function () {
            var flag = $('#selectuvfy').val();
            if ($(this).is(":checked")) {
                $('#pv_rost_unverfAsstFlag').val(flag);
                $('#selectrjct').prop('disabled', 'disabled');
            } else {
                $('#pv_rost_unverfAsstFlag').val('');
        ...
        $("#selectrjct").change(function () {
            ...
            if ($(this).is(":checked")) {
                $('#pv_rost_unverfAsstFlag').val(flag);
                $('#selectuvfy').prop('disabled', 'disabled');
                $('#remarkid').hide();
                $('#resonid').show();
            } else {
                $('#pv_rost_unverfAsstFlag').val('');
                $('#selectuvfy').prop('disabled', false);
                $('#remarkid').show();
                $('#resonid').hide();
            }
        });
