...
<script>
    $(document).ready(function () {
        $("#sanstha").hide();
            $("input[name=sel_sans]").on("change", function () {
            var sans = $('input:radio[name=sel_sans]:checked').val();
            alert(sans);
            if (sans == '1') {
                $("#sanstha").show();
            }
...
<div>
    <?php echo $this->Form->create('rep_sans_vaca', array('url' => array('controller' => 'Pavitras', 'action' => 'PvSansthaVacaPdf'),'target' => '_blank' , 'enctype' => 'multipart/form-data')); ?>
