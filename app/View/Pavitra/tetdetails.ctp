...
<?php echo $this->Form->create('apply_posn', array('url' => array('controller' => 'Pavitras', 'action' => 'savetetdetails'))); ?>
<div>
    <fieldset class="myfieldset">
        <legend><div class="legend_title"><?php echo __('TET Details'); ?></div></legend>
        <div class="table-responsive fielddata">
            <table class="table b_table" id="fieldtable">
                <tr>             
                    <td class="col-xs-2" colspan="2">
                        <?php echo __('Appeared For TET?'); ?><span style="float:right;">:&nbsp;&nbsp;&nbsp;</span>
                    </td>
        ...
        <fieldset class="myfieldset">
            ...
            <table class="table b_table" id="tetpapper2">
                <tr>
                    ...
                    <td class="col-xs-3" colspan="3">
                        <?php if (!empty($check[0]['TetExmDet']['pv_exm_id_2'])) { ?>
...
<script>
    $(document).ready(function() {
        ...
        $('#tetpappernumber1').on('change', function() {
            var papperoneid = $('#tetpappernumber1').val();
            var x = $("input[name='tet']:checked").val();
            var paperone = $("input[name='tetpapper1']:checked").val();
//            alert(paperone);
            jQuery.post('checkpapperonedata', {papperoneid: papperoneid, tet: x, paperone: paperone}, function(data) {
                $('#tetpapper1').html(data);
            });
        });
        ...
        $('#tetpappernumber2').on('blur', function() {
            ...
            var x = $("input[name='tet']:checked").val();
            var papertwo = $("input[name='tetpapper1']:checked").val();
            jQuery.post('checkpappertwodata', {pappertwoid: pappertwoid, tet: x, papertwo: papertwo}, function(data) {
                $('#tetpapper2').html(data);
            });
        });

        $('#tetpappernumber2').on('change', function() {
            var pappertwoid = $('#tetpappernumber2').val();
            var x = $("input[name='tet']:checked").val();
            var papertwo = $("input[name='tetpapper1']:checked").val();
//            alert(paperone);
    ...
    $("#tetpappernumber1").keydown(function(e) {
    ...
    $("#tetpappernumber2").keydown(function(e) {
        if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 || (e.keyCode == 65 && (e.ctrlKey === true || e.metaKey === true)) || (e.keyCode == 67 && (e.ctrlKey === true || e.metaKey === true)) || (e.keyCode == 88 && (e.ctrlKey === true || e.metaKey === true)) || (e.keyCode >= 35 && e.keyCode <= 39)) {
            return;
        }
        if ((e.shiftKey || (e.keyCode < 48 || e.keyCode > 57)) && (e.keyCode < 96 || e.keyCode > 105)) {
